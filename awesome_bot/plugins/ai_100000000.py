...
ai = on_command("ai", rule=to_me(), priority=6)
...
async def get_replay(sentence: str):
    ...
    sentence = re.sub(r'[你]', "\a", sentence)
    sentence = re.sub(r'[我]', "你", sentence)
    sentence = re.sub(r'\a', "我", sentence)
    sentence = re.sub(r'you', "me", sentence)
    sentence = re.sub(r'是不是', "是", sentence)
    # print('called!')
    return sentence
